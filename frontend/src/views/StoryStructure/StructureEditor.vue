<template>
  <div class="structure-editor-view" v-loading="isLoadingOverall" element-loading-text="加载项目结构...">
    ...
    <el-alert v-if="chapterStore.error || sceneStore.error" :title="`加载结构数据时出错: ${chapterStore.error || sceneStore.error}`" type="error" show-icon closable @close="clearErrors"/>
            ...
            <template #item="{ element: chapter }">
              <el-card shadow="never" class="chapter-item" :class="{ 'is-active': selectedChapterId === chapter.id }">
                <div class="chapter-header" @click="selectChapter(chapter.id)">
                   ...
                   <span class="chapter-title">{{ chapter.title || '未命名章节' }} ({{ chapter.scenes?.length || 0 }} 场景)</span>
                   <div class="chapter-actions">
                     <el-button link type="primary" :icon="Edit" @click.stop="openEditChapterDialog(chapter)" size="small"></el-button>
                     <el-button link type="danger" :icon="Delete" @click.stop="confirmDeleteChapter(chapter)" size="small"></el-button>
                     <el-button link type="success" :icon="DocumentAdd" @click.stop="openCreateSceneDialog(chapter.id)" size="small" title="在本章新建场景"></el-button>
                   </div>
                </div>
    ...
    <!-- 对话框：创建/编辑章节 -->
    <el-dialog v-model="chapterDialogVisible" :title="isEditingChapter ? '编辑章节' : '新建章节'" width="500px" @closed="resetChapterForm">
        ...
        if (targetChapterId !== null && targetChapterId === sourceChapterId) {
            ...
            if (chapter && chapter.scenes) {
               const sceneUpdates = chapter.scenes.map((scene, index) => ({
                   id: scene.id,
                   updateData: { order_in_chapter: index }
               })).filter(u => {
